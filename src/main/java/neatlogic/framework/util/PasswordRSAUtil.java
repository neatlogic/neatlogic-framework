...
public final class PasswordRSAUtil {
    ...
    /** 兼容升级前由RSA直接加密密码所使用的密文前缀。 */
    private static final String LEGACY_RSA_ENCRYPTED_PREFIX = "RSA:";
...
//    }
    ...
    public static int getMaxPlaintextByteLength() {
        ...
        return MAX_PLAINTEXT_BYTE_LENGTH;
    }
    ...
    public static boolean isEncrypted(String password) {
        return password != null && (password.startsWith(ENCRYPTED_PREFIX) || password.startsWith(LEGACY_RSA_ENCRYPTED_PREFIX));
    }

    /**
     * 使用随机AES-GCM密钥加密密码，再使用RSA-OAEP公钥加密AES密钥。
     *
     * @param plainPassword 待加密的明文密码
     * @return AES:密码载荷.RSA:AES密钥密文格式的混合密文
     */
    public static String encrypt(String plainPassword) {
        ...
        try {
            ...
            byte[] encryptedAesKey = encryptAesKeyByRsa(aesKey);
    ...
    /**
     * 解密前端提交的AES-GCM与RSA混合密文，并兼容升级前的RSA密码密文。
     */
    public static String decrypt(String encryptedPassword) {
        ...
        try {
            if (encryptedPassword.startsWith(ENCRYPTED_PREFIX)) {
                return decryptHybridPassword(encryptedPassword);
            }
            // 非AES格式按旧RSA协议处理，兼容升级前已打开页面回传的RSA密文。
            String rsaCiphertext = encryptedPassword.startsWith(LEGACY_RSA_ENCRYPTED_PREFIX)
                    ? encryptedPassword.substring(LEGACY_RSA_ENCRYPTED_PREFIX.length())
                    : encryptedPassword;
            return decryptLegacyPassword(Base64.getDecoder().decode(rsaCiphertext));
        } catch (Exception e) {
     ...
     */
    private static byte[] encryptAesKeyByRsa(SecretKey aesKey) throws GeneralSecurityException {
     ...
     */
    private static SecretKey decryptAesKeyByRsa(byte[] encryptedAesKey) throws GeneralSecurityException {
        Cipher rsaCipher = Cipher.getInstance(RSA_CIPHER_TRANSFORMATION);
