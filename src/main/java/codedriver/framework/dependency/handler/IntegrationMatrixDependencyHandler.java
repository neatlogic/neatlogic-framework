    ...
    protected ValueTextVo parse(Object caller) {
        if (caller instanceof String) {
            ...
            if (matrixVo != null) {
                ...
                valueTextVo.setText(String.format("<a href=\"/%s/framework.html#/matrix-external-edit?uuid=%s&name=%s&type=%s\" target=\"_blank\">%s</a>", TenantContext.get().getTenantUuid(), matrixVo.getUuid(), matrixVo.getName(), matrixVo.getType(), matrixVo.getName()));
                return valueTextVo;
            }
        }
        return null;
