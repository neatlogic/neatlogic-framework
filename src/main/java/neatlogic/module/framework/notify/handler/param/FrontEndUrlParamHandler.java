 ...
 **/
@Component
public class FrontEndUrlParamHandler implements INotifyParamHandler {

    @Override
    public String getValue() {
        return CommonNotifyParam.FRONT_END_URL.getValue();
    }

    @Override
    public Object getText(Object object) {
        String frontEndUrl = Config.FRONT_END_URL();
        if(StringUtils.isNotBlank(frontEndUrl)) {
            if(!frontEndUrl.endsWith("/")) {
                frontEndUrl += "/";
            }
            return frontEndUrl + TenantContext.get().getTenantUuid() + "/";
        }
        return null;
    }
}
