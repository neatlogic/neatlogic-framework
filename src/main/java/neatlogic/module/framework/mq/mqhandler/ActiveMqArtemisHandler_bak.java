...
//@Component
public class ActiveMqArtemisHandler_bak implements IMqHandler {
    private static final Logger logger = LoggerFactory.getLogger(ActiveMqArtemisHandler_bak.class);
    ...
    public boolean create(SubscribeVo subVo) throws SubscribeTopicException {
        ...
        if (!containerMap.containsKey(subVo.getId())) {
            ...
            container.setPubSubDomain(false);//使用队列模式，多节点竞争消费
    ...
    public void send(String topicName, String content) {
        try {
            jmsTemplate.convertAndSend("queue://" + TenantContext.get().getTenantUuid() + "/" + topicName.toLowerCase(), content);
        } catch (Exception ex) {
