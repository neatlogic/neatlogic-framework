...
public class AuditUtil {
    ...
    public static void saveAuditDetail(AuditVoHandler vo, String fileType) {
        /**
         * 组装文件内容JSON并且计算文件中每一块内容的开始坐标和偏移量 例如参数的开始坐标为"param>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"的字节数
        ...
        if (StringUtils.isNotBlank(vo.getParam())) {
            ...
            vo.setParamFilePath(
                "?startIndex=" + sb.toString().getBytes(StandardCharsets.UTF_8).length + "&offset=" + offset);
        ...
        /**
         * 用一个单独的流存result，避免append到StringBuilder后增加内存负担 然后利用SequenceInputStream合并两个流
         */
        InputStream resultInputStream = null;
        String resultStr = null;
        if (vo.getResult() != null) {
            resultStr = JSON.toJSON(vo.getResult()).toString();
        }
        ...
        try {
            filePath = FileUtil.saveData(MinioFileSystemHandler.NAME, TenantContext.get().getTenantUuid(), sis,
                SnowflakeUtil.uniqueLong(), "text/plain", fileType);
        } catch (Exception e) {
            // logger.error(e.getMessage(),e);
            try {
                filePath = FileUtil.saveData(LocalFileSystemHandler.NAME, TenantContext.get().getTenantUuid(), sis,
                    SnowflakeUtil.uniqueLong(), "text/plain", fileType);
            } catch (Exception e1) {
        ...
        if (StringUtils.isNotBlank(filePath)) {
            ...
            if (length != 0) {
                ...
                if (StringUtils.isNotBlank(resultStr)) {
                    /**
                     * 计算result在文件中的起始位置和偏移量 偏移量 = 文件总长度 - 截止到result开始定界符的长度 - result结束定界符的长度
                     */
                    long resultOffset = length - lengthWithoutResult
                        - "result<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<".getBytes(StandardCharsets.UTF_8).length - 1;
        ...
        try {
            ...
            if (in != null) {
                ...
                while ((len = in.read(buff)) != -1) {
                    /**
                     * endPoint用来记录累计读取到的字节数 如果大于偏移量，说明实际读到的数据超过了需要的数据 那么就需要减掉多读出来的数据
                     */
                    endPoint += len;
                    if (endPoint > offset) {
